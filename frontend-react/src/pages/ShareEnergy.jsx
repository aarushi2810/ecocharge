...
export default function ShareEnergy() {
  ...
  const listings = useMemo(() => {
    let nextListings = [...energyListings];

    if (filterBy === "high") {
      nextListings = nextListings.filter((item) => item.energy >= 18);
    }

    if (filterBy === "budget") {
      nextListings = nextListings.filter((item) => item.price <= 7.2);
    }

    if (sortBy === "price") {
      nextListings.sort((a, b) => a.price - b.price);
    }

    if (sortBy === "energy") {
      nextListings.sort((a, b) => b.energy - a.energy);
    }

    if (sortBy === "distance") {
      nextListings.sort((a, b) => a.distance - b.distance);
    }
    ...
    <div className="page-share-energy">
      <section className="share-energy-head">
        <div>
          ...
          <p>
            Browse local apartment communities, compare rates, and request access
            to available solar energy capacity.
          </p>
        </div>
